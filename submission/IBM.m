function IBM(rootdir)
    % IBM('Testing') to run this under /Testing
    %
    %
    
    flac_files = dir([rootdir, '/unkn*.flac']);
    text_files = dir([rootdir, '/unkn*.txt']);
    ...
    for file_idx = 1:n_flac
        ...
        % Do some stuff
        cmd = ['env LD_LIBRARY_PATH="" curl -u 2b78670c-c890-477b-a54b-a4caeb27ad0e:JJgGDcOVXKxB -X POST --header "Content-Type: audio/flac" --header "Transfer-Encoding: chunked" --data-binary @', rootdir,'/', flac_files(file_idx).name, ' "https://stream.watsonplatform.net/speech-to-text/api/v1/recognize?continuous=true"'];
        [x, y] = unix(cmd);
        ...
        [xxx1, xxx2, hypothesis] = textread([rootdir, '/', text_files(file_idx).name], '%d %d %s', 'delimiter','\n');
